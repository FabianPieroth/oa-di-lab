...
class ConvLayer (nn.Module):
    def __init__(self, c_in, c_out, stride, kernel_size, padding, drop_prob, groups):
        super().__init__()
        self.conv = nn.Conv2d(c_in,c_out,stride=stride, kernel_size=kernel_size, padding=padding, groups=groups).double()
...
class DeConvLayer(nn.Module):
    ...
    def __init__(self, c_in, c_out, stride, kernel_size, padding, output_padding, drop_prob):
        super().__init__()
        self.deconv = nn.ConvTranspose3d(c_in, c_out, stride=stride, kernel_size=kernel_size,
